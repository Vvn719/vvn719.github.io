const DEFAULT_SEMESTERS = [{ id: 'sem-114-down', name: '114下培緣', year: '114', term: '下', status: 'active' }]
...
let currentStudents = DEFAULT_STUDENTS.map((item, index) => ({
  ...item,
  apiId: item.apiId || `student-${String(index + 1).padStart(3, '0')}`,
  sortOrder: index + 1,
  active: true
}))
...
function normalizeStudent(item, index) {
  const activeValue = String(item.active ?? 'TRUE')
    .trim()
    .toLowerCase()
  return {
...
function loadDataCache(semesterId = currentSemesterId) {
  try {
    const raw = localStorage.getItem(dataCacheKey(semesterId)) || (semesterId === DEFAULT_SEMESTERS[0].id ? localStorage.getItem('attendanceDataCache:v1') : null)
...
async function reloadFromApi(options = {}) {
  ...
  syncInFlight = performApiSync({ ...options, semesterId: requestedSemesterId }).finally(() => {
    syncInFlight = null
...
function openClassEditModal(classId) {
  ...
  const semester = typeof currentSemesterLabel === 'function' ? currentSemesterLabel() : currentSemesterId

  document.getElementById('classEditMeta').textContent = `目前學期：${semester}｜課程 ID：${classItem.id}`
...
async function saveClassEdit() {
  ...
  const dateInput = document.getElementById('classEditDateInput')
  const titleInput = document.getElementById('classEditTitleInput')
  const sortOrderInput = document.getElementById('classEditSortOrderInput')
  ...
  const errors = []
  if (!date) errors.push('請填寫上課日期')
  if (!title) errors.push('請填寫課程名稱')
  if (!isPositiveInteger(sortOrderValue)) errors.push('排序必須是大於 0 的整數')

  if (errors.length) {
    errorEl.innerHTML = errors.map(item => `<div>${item}</div>`).join('')
    errorEl.classList.add('show')

    if (!date) dateInput.focus()
    else if (!title) titleInput.focus()
    else sortOrderInput.focus()

    setAdminStatus(errors[0], 'danger')
    return
  }

  if (!apiUrl) {
    errorEl.textContent = '請先設定 Apps Script API URL。'
    errorEl.classList.add('show')
    configureApi()
    return
  }

  const saveBtn = document.getElementById('classEditSaveButton')
  ...
  try {
    ...
    console.error(error)
    errorEl.textContent = `課程更新失敗：${error.message || '未知錯誤'}`
    errorEl.classList.add('show')
    setAdminStatus(`課程更新失敗：${error.message || '未知錯誤'}`, 'danger')
  } finally {
...
function renderSemesterOptions() {
  const options = semesters.map(item => `<option value="${escapeHtml(item.id)}">${escapeHtml(item.name || item.id)} (${escapeHtml(item.id)})</option>`).join('')
...
async function confirmCsvImport(type) {
  ...
  if (
    !(await showConfirm(
      `確定將 ${state.rows.length} 筆${config.label}資料匯入「${currentSemesterLabel()}」？\n這會替換 Google Sheet 目前學期的${config.label}清單。`,
      `匯入${config.label}`
    ))
  )
    return
...
function findStudent(key) {
  return currentStudents.find(student => student.key === key)
}
function classEndColumn() {
  return 5 + currentClasses.length * 3
}
function summaryStartColumn() {
  return classEndColumn() + 2
}
function totalExportColumns() {
  return summaryStartColumn() + 2
}
function classRecordKey(index = classSelect.value || 0) {
  return String(index)
}
function currentClass() {
  return currentClasses[Number(classSelect.value || 0)] || currentClasses[0]
}
function classIndexById(classId) {
  return currentClasses.findIndex(item => item.id === classId)
}
function isExcluded(student) {
  return Boolean(student && excluded.has(student.key))
}
function isPresent(student) {
  if (!student || isExcluded(student)) return false
  const special = specialNotes.get(student.key)
  return selected.has(student.key) || special?.type === 'online' || special?.type === 'late'
}
function isAbsent(student) {
  if (!student || isExcluded(student)) return false
  return specialNotes.get(student.key)?.type === 'absent'
}
function isDone(student) {
  return confirmed.has(student.key) || isAbsent(student) || isExcluded(student)
}
function isEditingDone(student) {
  return editingDone.has(student.key)
}
function isLocked(student) {
  return isDone(student) && !isEditingDone(student)
}
...
function roleClass(role) {
  if (role.includes('講師') || role.includes('點傳師')) return 'text-bg-danger'
  if (role === '輔導') return 'text-bg-warning'
  return 'text-bg-success'
}
function specialLabel(type) {
  return { online: '線上', late: '遲到', absent: '缺席' }[type] || ''
}
function specialBadgeClass(type) {
  return { online: 'text-bg-info', late: 'text-bg-warning', absent: 'text-bg-secondary' }[type] || 'text-bg-light'
}
...
function specialTextFor(group) {
  const list = currentStudents
    .filter(student => student.group === group && !isExcluded(student) && specialNotes.has(student.key))
    .map(student => ({ ...student, special: specialNotes.get(student.key) }))
...
function renderSummary() {
  ...
  document.getElementById('summaryArea').innerHTML = `
        <div class="card summary-card shadow-sm"><div class="card-body">
          ...
          <div class="mt-4"><h5 class="fw-bold mb-2">特殊情況</h5><div class="table-responsive"><table class="table table-sm table-bordered align-middle mb-0 table-summary"><thead class="table-light"><tr><th style="width: 7rem;">組別</th><th>特殊情況條列</th></tr></thead><tbody>${reportGroups()
            .map(group => `<tr><th>${group}</th><td>${specialTextFor(group)}</td></tr>`)
...
function toggleStudent(key, isChecked) {
  if (specialNotes.get(key)?.type === 'absent' || excluded.has(key)) return
  if (confirmed.has(key) && !isChecked) {
    selected.add(key)
    renderList()
    return
  }
...
function cancelAbsentEdit(key, event) {
  event.stopPropagation()
  editingAbsent.delete(key)
  renderList()
}
...
function toggleAdvanced(key, event) {
  event.stopPropagation()
  if (advancedOpen.has(key)) advancedOpen.delete(key)
  else advancedOpen.add(key)
  renderList()
}
...
function checkAllVisible() {
  visibleIds.forEach(key => {
    if (specialNotes.get(key)?.type !== 'absent' && !excluded.has(key)) selected.add(key)
  })
  renderList()
}
...
function classStartColumn(index) {
  return 6 + index * 3
}
